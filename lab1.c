...
int checkLenght(float a)
{
	...
	if (a < 0)                                                                              //начало перевірки числа темп щоб не було 0
		n++;
	for (;;)                                                                                //нескінченна кількість разів
	{
...
double func(float x)                                                                       //функція double имеет 15 десятичных цифр точности, в то время как float имеет 7
{
	return sin(x) * cos(x);
}
